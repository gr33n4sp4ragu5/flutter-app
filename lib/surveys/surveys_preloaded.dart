...
class PreloadedSurveys {
  ...
  void preloadSurveys() async {
    ...
    SurveyState read_mid_eyes_survey_state = await getCurrentSurveyState("ReadMindEyes");
    SurveyState tds_state = await getCurrentSurveyState("TeamDiagnosticSurvey");
    CIMSurvey read_mind_eyes = new CIMSurvey(LinearSurveyPage(), "ReadMindEyes", read_mid_eyes_survey_state, "ninguno", "Reading the mind in the eyes");
    CIMSurvey team_diagnostic_survey = new CIMSurvey(TDSSurveyPage(), "TeamDiagnosticSurvey", tds_state, "ninguno", "Team Diagnostic Survey");
  ...
  Future<SurveyState> getCurrentSurveyState(String surveyId) async {
    ...
    final finished_surveys = await get(AppUrl.getProfileData,
